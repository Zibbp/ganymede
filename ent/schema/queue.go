...
func (Queue) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("task_chat_render").GoType(utils.TaskStatus("")).Default(string(utils.Waiting)).Optional(),
		field.Enum("task_chat_move").GoType(utils.TaskStatus("")).Default(string(utils.Waiting)).Optional(),
