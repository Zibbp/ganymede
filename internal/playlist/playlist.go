...
func (s *Service) GetPlaylist(c echo.Context, playlistID uuid.UUID) (*ent.Playlist, error) {
	rPlaylist, err := s.Store.Client.Playlist.Query().Where(playlist.ID(playlistID)).WithVods(func(vq *ent.VodQuery) { vq.WithChannel() }).Order(ent.Desc(playlist.FieldCreatedAt)).Only(c.Request().Context())
