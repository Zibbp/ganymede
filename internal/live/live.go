...
type Live struct {
	...
	BlacklistCategories    bool                 `json:"blacklist_categories_live"` // Blacklist selected categories for live streams and videos.
	...
	for _, lwc := range liveWatchedChannels {
		...
		if len(stream.ID) > 0 {
			...
			if !lwc.IsLive {
				...
				if lwc.ApplyCategoriesToLive && len(lwc.Edges.Categories) > 0 && !lwc.BlacklistCategories {
					if _, found := watchedChannelCategories[strings.ToLower(stream.GameName)]; found {
						log.Debug().Str("category", stream.GameName).Str("category_restrictions", strings.Join(categoryNamesForLog, ", ")).Msgf("%s matches category restrictions", lwc.Edges.Channel.Name)
					} else {
				...
				// check for blacklist category restrictions
				if lwc.ApplyCategoriesToLive && lwc.BlacklistCategories && len(lwc.Edges.Categories) > 0 {
					if _, found := watchedChannelCategories[strings.ToLower(stream.GameName)]; found {
